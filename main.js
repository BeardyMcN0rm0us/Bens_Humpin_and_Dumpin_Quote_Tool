// r309 — UI toggle + pricing + Maps + WhatsApp (robust)
...
(function(){
  ...
  // ---- ensure required blocks exist (so toggling always works)
  ...
  function scaffold(){
    ...
    ensure('pickupField',  `<label for="addrPickup">Collection address</label><input id="addrPickup" type="text" placeholder="Start typing…">`);
    ensure('addrDropWrap', `<label for="addrDrop">Delivery address</label><input id="addrDrop" type="text" placeholder="Start typing…">`);
    ensure('wasteWrap',    `<label for="wasteType">Waste type (Tip runs)</label><select id="wasteType"></select><div class="hint">We use the minimum disposal fee (25% of £/t).</div>`);
    ensure('twoManWrap',   `<label for="twoMan">Two‑person team?</label><select id="twoMan"><option value="no">No</option><option value="yes">Yes</option></select>`);
    ensure('stairsWrap',   `<div class="grid2"><div><label for="stairsPickup">Stairs at pickup</label><input id="stairsPickup" type="number" value="0"></div><div><label for="stairsDrop">Stairs at drop‑off</label><input id="stairsDrop" type="number" value="0"></div></div>`);
    ensure('shopTimeWrap', `<label for="shopTime">Run time</label><select id="shopTime"><option value="before22">Before 10pm</option><option value="after22">After 10pm</option></select>`);
    ensure('ikeaModeWrap', `<label for="ikeaMode">IKEA service</label><select id="ikeaMode"><option value="collect">Collect Only</option><option value="collectBuild">Collect & Build</option></select>`);
    ensure('ikeaStoreWrap',`<label for="ikeaStore">IKEA store</label><select id="ikeaStore"></select>`);
    ensure('ikeaQtyWrap',  `<label for="ikeaQty">Items to build</label><input id="ikeaQty" type="number" value="1" min="1">`);
    // results & actions (if not already present)
  ...
  function setUI(){
    ...
    else if (v==='ikea'){ show(els.addrDropWrap); show(els.ikeaModeWrap); show(els.ikeaStoreWrap); show(els.twoManWrap); show(els.stairsWrap); show(els.descWrap); if(els.ikeaMode && els.ikeaMode.value==='collectBuild') show(els.ikeaQtyWrap); if(els.routeHint) els.routeHint.textContent="Mileage: Home → IKEA → Delivery."; }
  ...
  // Maps (optional; still works if key blocked)
  let directions=null;
  function initMaps(){
    try{
