...
const rm=v=>round5(v); // roundMoney — rounds every £ value to nearest £5
...
o.value=k; o.textContent=`${it.label} (${fmtMoney(it.ratePerTonne)}/t + VAT)`; els.wasteType.appendChild(o);
});
}
...
els.lutonHint.textContent=txt+` Hire used: ${fmtMoney((els.lutonCost&&els.lutonCost.value)||CFG.LUTON_HIRE_COST)}/day.`;
}
...
if(jt===“bags”||jt===“business”) return 0;
...
return{fee:minFee,detail:`Disposal: ${item.label||key} — 25% of ${fmtMoney(item.ratePerTonne)}/t = ${fmtMoney(exVat)} + VAT ${fmtMoney(vat)}`};
}
...
`${count} bag${count!==1?'s':''} @ ${fmtMoney(priceEach)}/bag = ${fmtMoney(bagCost)}`,
...
`${bales} bales for sale @ ${fmtMoney(salePerBale)}/bale = ${fmtMoney(saleCost)}`,
...
`${bales} bales${isFullLoad?' (full load)':` (min ${minBales})`} rental @ ${fmtMoney(rentalPerBalePerDay)}/bale/day = ${fmtMoney(rentalCost)}`,
`Mileage charged for return trip (delivery + collection)`,
`⚠️ Damaged/wet bales: ${fmtMoney(damagedFee)}/bale — if wet, you keep them`,
]};
}
}
...
cost=totalMinutes*perMin; txt=` (~${fmtMins(totalMinutes)} @ ${fmtMoney(perHour)}/hour)`;
}else if(totalItems>0){
const perItem=Number(CFG.ikeaAssemblyPerItem||15);
cost=totalItems*perItem; txt=` (${totalItems} × ${fmtMoney(perItem)}/item)`;
}
...
if(jt==='move'){
  ...
  if(map){
    ...
    labourLine=`${map.hours} hrs labour @ ${fmtMoney(CFG.HOURLY_RATE_MOVE)}/hr = ${fmtMoney(labourCost)}`;
  }
  ...
  if(include){
    ...
    lutonLine=`Luton van hire: ${fmtMoney(lutonCost)} (${mode==='auto'?'auto':mode==='yes'?'forced':'overridden OFF'})`;
  }else if(mode==='no'){
...
if(jt==='bags'){
  ...
  lines.push(`Charged mileage: ${chargedMiles.toFixed(1)} miles @ £${Number(CFG.mileagePerMile).toFixed(2)}/mile`);
  lines.push(`Base fee: ${fmtMoney(base)}`);
  lines.push(`Mileage: ${fmtMoney(mileageCost)}`);
}

if(stairs) lines.push(`Stairs: ${fmtMoney(stairs)}`);
if(twoMan){
  const beds=parseInt(els.houseMoveBedrooms&&els.houseMoveBedrooms.value||'0',10);
  const map=CFG.BEDROOM_LOAD_MULTIPLIERS[beds];
  lines.push(`Two-person helper: ${fmtMoney(twoMan)}${jt==='move'&&map?` (${map.hours} hrs @ ${fmtMoney(CFG.twoManSurcharge)}/hr)`:' (flat fee)'}`);
}
...
if(asm.cost){
  ...
  lines.push(`Assembly: ${fmtMoney(asm.cost)}${asm.txt}`);
}
if(jt==='move'){
