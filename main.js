...
return{fee:minFee,detail:`Disposal: ${item.label||key} — 25% of £${Number(item.ratePerTonne||0).toFixed(2)}/t = ${fmtMoney(exVat)} + VAT ${fmtMoney(vat)}`};
}
...
`${count} bag${count!==1?'s':''} @ £${priceEach}/bag = ${fmtMoney(bagCost)}`,
...
`${bales} bales for sale @ £${salePerBale}/bale = ${fmtMoney(saleCost)}`,
...
`${bales} bales${isFullLoad?' (full load)':` (min ${minBales})`} rental @ £${rentalPerBalePerDay}/bale/day = ${fmtMoney(rentalCost)}`,
...
cost=totalItems*perItem; txt=` (${totalItems} x ${fmtMoney(perItem)}/item)`;
}
...
lines.push(`Total journey: ${loopMiles.toFixed(1)} miles (${noteL||(jt==='flatpack'?'Home to Destination and back':'')})`);
